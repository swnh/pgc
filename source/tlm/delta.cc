    ...
    while (true) {
        ...
        if (is_rdy && vld_in) {
            if (points_received == 0) {
                std::cout << "@" << sc_time_stamp() << " Packet starts to send (FIFO filling). First point laser_id=" << d_in.range(52, 48) << std::endl;
            }
        ...
        switch (state) {
            case IDLE:
                ...
                if (fifo_count > 0 && points_processed < 384) {
                    ...
                    std::cout << "@" << sc_time_stamp() << " History buffer read at index " << laser_id << std::endl;
            ...
            case CALC:
                std::cout << "@" << sc_time_stamp() << " Delta calculation happens" << std::endl;
            ...
            case WRITE_OUT:
                std::cout << "@" << sc_time_stamp() << " History buffer write at index " << laser_id << std::endl;
            ...
            case OUTPUT:
                ...
                if (rdy_out) {
                    ...
                    if (points_processed == 384) {
                        // End of packet 
                        std::cout << "@" << sc_time_stamp() << " Finished to process the packet" << std::endl;
                        points_received = 0;
                        points_processed = 0;
                        next_state = IDLE;
